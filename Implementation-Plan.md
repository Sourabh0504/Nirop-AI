...
6. **Deliverability circuit breaker** — mid-campaign, if failure or unsubscribe rate crosses a threshold, auto-pause remaining sends and alert. Directly protects domain reputation (the plan's stated #1 priority) — ranks above the fancier AI features below. ✅ Done — uses SMTP failure rate as a proxy since real bounce classification (milestone 10) isn't built yet; swap in true bounce rate once IMAP parsing exists.
